...
type OAuthState struct {
	...
	RedirectURI   string
	ClientID      string
	Username      string // OMS username
	Password      string // OMS password
	CreatedAt     time.Time
	AuthCode      string
	Client        *client.Client // Authenticated OMS client
	ExpiresAt     time.Time
}
...
type OAuthClient struct {
	ClientID     string
	ClientSecret string
	Username     string // OMS username (from client_id)
	Password     string // OMS password (from client_secret)
	RedirectURIs []string
	CreatedAt    time.Time
}

// OAuthManager manages OAuth 2.0 with PKCE flow for OMS
...
func NewOAuthManager(baseURL, omsIP string) *OAuthManager {
...
func (m *OAuthManager) HandleRegister(w http.ResponseWriter, r *http.Request) {
	...
	// Extract client_id (OMS username) and client_secret (OMS password)
	var clientID string
	...
	// Generate client_id if still not provided
	if clientID == "" {
	...
	client := &OAuthClient{
		...
		Username:     clientID,     // client_id is the OMS username
		Password:     clientSecret, // client_secret is the OMS password
...
func (m *OAuthManager) HandleAuthorize(w http.ResponseWriter, r *http.Request) {
	...
	// Allow localhost and Claude redirects for public clients
	if !validRedirect {
...
func (m *OAuthManager) HandleToken(w http.ResponseWriter, r *http.Request) {
	...
	if clientID != "" && clientSecret != "" {
		if clientObj, exists := m.clients.Load(clientID); exists {
			...
			client.Password = clientSecret // client_secret is the OMS password
			if client.Username == "" {
				client.Username = clientID // client_id is the OMS username
			}
			m.clients.Store(clientID, client)
...
func (m *OAuthManager) handleAuthorizationCodeGrant(w http.ResponseWriter, code, redirectURI, codeVerifier, clientID string) {
	...
	// Get client to extract credentials
	...
	password := oauthClient.Password

	// If not set in client, use clientID as username
	if username == "" {
		username = oauthState.ClientID
	}

	// Validate we have credentials
	if password == "" {
		m.sendTokenError(w, "invalid_client", "Client secret (OMS password) required. Configure client_secret in Claude Desktop OAuth settings.")
		return
	}

	fmt.Printf("[OAuth] Token: Logging in to OMS with username=%s\n", username)

	// Login to OMS
	ctx := context.Background()
	omsClient, err := client.NewClient(m.baseURL, m.omsIP, username, password)
	if err != nil {
		fmt.Printf("[OAuth] Token: Failed to create OMS client: %v\n", err)
		m.sendTokenError(w, "server_error", fmt.Sprintf("Failed to create OMS client: %v", err))
		return
	}

	if err := omsClient.Login(ctx); err != nil {
		fmt.Printf("[OAuth] Token: OMS login failed: %v\n", err)
		m.sendTokenError(w, "invalid_grant", fmt.Sprintf("OMS login failed: %v. Check your credentials in Claude Desktop OAuth settings.", err))
		return
	}

	fmt.Printf("[OAuth] Token: OMS login successful\n")
	...
	// Store session with the authenticated OMS client
...
func (m *OAuthManager) handleRefreshTokenGrant(w http.ResponseWriter, refreshToken, clientID string) {
	...
	// Re-authenticate with OMS to get fresh session
	ctx := context.Background()
	omsClient, err := client.NewClient(m.baseURL, m.omsIP, oauthState.Username, oauthState.Password)
	if err != nil {
		m.sendTokenError(w, "server_error", "Failed to create OMS client")
		return
	}

	if err := omsClient.Login(ctx); err != nil {
		m.sendTokenError(w, "invalid_grant", fmt.Sprintf("OMS re-authentication failed: %v", err))
		return
	}
...
// GetClient retrieves an authenticated OMS client by access token
